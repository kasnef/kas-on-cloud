...
__export(index_exports, {
  ...
  generateMicrosoftAccessToken: () => generateMicrosoftAccessToken,
  getDocumentLibraryId: () => getDocumentLibraryId,
  getItemListFromSharepoint: () => getItemListFromSharepoint,
  getSiteId: () => getSiteId,
  multiUploadToSharepoint: () => multiUploadToSharepoint,
...
async function uploadToSharePoint(accessToken, tenantName, siteName, fileName, fileContent, isShowLog = false, folderPath = "") {
  ...
  const response = await import_axios.default.put(url, fileContent, {
    headers: {
      Authorization: `Bearer ${accessToken}`,
...
async function generateMicrosoftAccessToken(config, isShowLog = false) {
  ...
  const res = await import_axios2.default.post(tokenUrl, data, {
    headers,
    auth: {
      username: clientId,
